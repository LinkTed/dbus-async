...
impl Connection {
    ...
    fn find_sender_by_object_path(&mut self, msg: Message) -> Option<Message> {
        if let Some(object_path) = msg.get_path() {
    ...
    fn find_sender_by_interface(&mut self, msg: Message) {
    ...
    pub(super) fn method_call(&mut self, msg: Message) {
        // A valid `MethodCall` must have a `Path` and a `Member`.
        if !msg.get_path().is_some() || !msg.has_member() {
            return;
        }
        ...
        if let Some(msg) = msg {
            ...
            // `Interface`.
            self.find_sender_by_interface(msg);
        }
    }
}
